...
export default function AdminGroups() {
  ...
  const handleDeleteGroup = (groupId: string) => {
    ...
    if ((group.userCount || 0) > 0) {
      console.log(
        `Cannot delete "${group.name}" because it has ${group.userCount} active users. Please reassign users first.`,
      );
      return;
    }
  ...
  const handleLicenseSubmit = async () => {
    ...
    try {
      ...
      console.log(
        `Successfully updated ${selectedGroup.name} license count from ${selectedGroup.licenses.toLocaleString()} to ${newCount.toLocaleString()}`,
      );
              ...
              <div className="p-6">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-slate-600">
                      Update license count for {selectedGroup.name}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    <label className="block text-sm font-medium text-slate-700 mb-2">
                      Current Licenses:{" "}
                      {selectedGroup.licenses.toLocaleString()}
                    </label>
                    <label className="block text-sm font-medium text-slate-700 mb-2">
                      New License Count
                    </label>
