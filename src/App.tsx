...
// [PERF-Phase4] MainLayout lazy-loaded to reduce initial JS evaluation (TBT)
// Pre-rendered shell in index.html ensures no layout shift during load.
const MainLayout = lazy(() => import("./layouts/MainLayout"));
import ProtectedRoute from "./components/ProtectedRoute";
// [TBT-FIX] Analytics imports deferred to AnalyticsWrapper via dynamic import
...
// [PERF-C1] Original eager import — commented out to reduce main bundle size
...
// [PERF-C1] Original eager imports — commented out to reduce main bundle size (~400KB saved)
...
// [PERF-C1] NetworkFile pages — converted from wildcard import to individual lazy imports
const NFQuickOverview = lazy(
  () => import("./pages/manual/network-file/QuickOverview"),
);
const NFInstallation = lazy(
  () => import("./pages/manual/network-file/Installation"),
);
const NFUserInterface = lazy(
  () => import("./pages/manual/network-file/UserInterface"),
);
const NFLicensing = lazy(() => import("./pages/manual/network-file/Licensing"));
const NFSettings = lazy(() => import("./pages/manual/network-file/Settings"));
const NFConnectingDomain = lazy(
  () => import("./pages/manual/network-file/ConnectingDomain"),
);
const NFCloudIntegration = lazy(
  () => import("./pages/manual/network-file/CloudIntegration"),
);
const NFErasingFiles = lazy(
  () => import("./pages/manual/network-file/ErasingFiles"),
);
const NFErasingTraces = lazy(
  () => import("./pages/manual/network-file/ErasingTraces"),
);
const NFSchedulingTasks = lazy(
  () => import("./pages/manual/network-file/SchedulingTasks"),
);
const NFReportManagement = lazy(
  () => import("./pages/manual/network-file/ReportManagement"),
);
const NFFAQ = lazy(() => import("./pages/manual/network-file/FAQ"));
const NFSupport = lazy(() => import("./pages/manual/network-file/Support"));
const NFAbout = lazy(() => import("./pages/manual/network-file/About"));
...
// [TBT-FIX] Analytics deferred to after first paint — reduces TBT by ~200-400ms
// These hooks and their modules are NOT needed for the initial render.
function AnalyticsWrapper({ children }: { children: React.ReactNode }) {
  const [analyticsReady, setAnalyticsReady] = useState(false);

  useEffect(() => {
    // Defer analytics init to after first paint using requestIdleCallback
    const scheduleAnalytics =
      typeof requestIdleCallback === "function"
        ? requestIdleCallback
        : (cb: () => void) => setTimeout(cb, 2000);

    scheduleAnalytics(() => setAnalyticsReady(true));
  }, []);

  useEffect(() => {
    if (!analyticsReady) return;
    // Dynamically import analytics modules — they're NOT in the initial bundle
    Promise.all([
      import("./utils/analytics"),
      import("./utils/microsoftClarity"),
    ])
      .then(([analyticsModule, clarityModule]) => {
        // Call .init() on the exported singleton instances (ga, clarity)
        analyticsModule.default?.init?.();
        clarityModule.default?.init?.();
      })
      .catch(() => {
        /* Silently fail */
      });
                  ...
                  <Routes>
                    <Route element={<MainLayout />}>
                      ...
                      <Route
                        ...
                        element={
                          <StatutoryRegulatoryComplianceDataErasureBlog />
                        }
                      />
                      <Route
                      ...
                      />
                      <Route
                        path="blog/esg-report"
                        element={<ESGReportBlog />}
                      />
                      ...
                      />
                      <Route
                        path="blog/hex-viewer"
                        element={<HexViewerBlog />}
                      />
                      <Route
                      ...
                      />
                      <Route
                        path="blog/vm-erasure"
                        element={<VMErasureBlog />}
                      />
                      <Route
                      ...
                      >
                        <Route
                          path="quick-overview"
                          element={<NFQuickOverview />}
                        />
                        <Route
                          path="installation"
                          element={<NFInstallation />}
                        />
                        <Route
                          path="user-interface"
                          element={<NFUserInterface />}
                        />
                        <Route path="licensing" element={<NFLicensing />} />
                        <Route path="settings" element={<NFSettings />} />
                        <Route
                          path="connecting-domain"
                          element={<NFConnectingDomain />}
                        />
                        <Route
                          path="cloud-integration"
                          element={<NFCloudIntegration />}
                        />
                        <Route
                          path="erasing-files"
                          element={<NFErasingFiles />}
                        />
                        <Route
                          path="erasing-traces"
                          element={<NFErasingTraces />}
                        />
                        <Route
                          path="scheduling-tasks"
                          element={<NFSchedulingTasks />}
                        />
                        <Route
                          path="report-management"
                          element={<NFReportManagement />}
                        />
                        <Route path="faq" element={<NFFAQ />} />
                        <Route path="support" element={<NFSupport />} />
                        <Route path="about" element={<NFAbout />} />
                      </Route>
                      <Route
                        path="/support/help-manual"
